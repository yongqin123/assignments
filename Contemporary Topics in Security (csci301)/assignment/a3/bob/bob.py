    ...
    def message(self, pubnub, message):
        ...
        if message.publisher == "alice":
            ...
            if blknum == 9:
                ...
                print("Bob picks " + str(rand_gen))
                abcdefghi.remove(rand_gen)
                hashed_block_int = (2**244)+1 # make it execute at least once

                while hashed_block_int > 2**244:
                    
                    block = json.dumps({'TxID': blknum+1, 'Hash': hashval, "Nonce" : nonce, "Transaction" : ["Bob", rand_gen]},indent=4, separators=(',', ': '),sort_keys=True)
